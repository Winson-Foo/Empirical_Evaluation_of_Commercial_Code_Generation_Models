...
def setup_logger():
    ...
    # You can log to a file using the `filename` parameter.
    # Or make the output more or less verbose by setting level to:
    # logging.DEBUG, logging.INFO, logging.WARNING, or logging.ERROR

    # Example:
    # logging.basicConfig(filename="/tmp/caiman.log", level=logging.WARNING)
...
def evaluate_components_cnn(opts, cnm, Cn):
    """
    Process CNN classifier with a low threshold
    """
    use_CNN = True
    if use_CNN:
...
def main():
    ...
    setup_logger()
    fname = load_data()
    opts = set_parameters(fname)
    cnm = fit_online(opts)
    plot_contours(cnm, fname)
    Cn = cm.load(fname[0], subindices=slice(0,500)).local_correlations(swap_dim=False)
    evaluate_components_cnn(opts, cnm, Cn)
    plot_results(cnm, Cn)
