...
def run_caiman_batch(opts, n_processes, dview):
    cnm = cnmf.CNMF(n_processes, params=opts, dview=dview)
    cnm = cnm.fit_file()
    return cnm


def evaluate_components(cnm, images, Cn):
...
def main():
    ...
    # set up parameters
    opts = setup_parameters()

    # run CaImAn Batch (CNMF)
    cnm = run_caiman_batch(opts, n_processes, dview)
    ...
    evaluate_components(cnm2, images, Cn)
    ...
    cnm2.estimates.view_components(
        images, idx=cnm2.estimates.idx_components, img=Cn
    )
