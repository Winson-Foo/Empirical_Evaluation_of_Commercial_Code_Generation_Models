...
def transversal_cuts(img, cuts, cmap='gray', normalize=False, axes=None):
    vmin, vmax = (img.min(), img.max()) if normalize else (0, 255)
...
def plot_patches_overlay(img, patches, selection=[], cmap='gray', normalize=False):
    ...
    for p in patches:
        if p.color == 'w':
            ...
            zorder = 2
        rect = mpatches.Rectangle((p.box[1], p.box[0]), p.box[2], p.box[3], linewidth=2, edgecolor=p.color, facecolor='none', zorder=zorder)
    ...
    if len(selection) > 0:
        ...
        grid = ImageGrid(fig, 122, nrows_ncols=(nrows, ncols), direction="row", axes_pad=0.15, add_all=True, share_all=True)
...
def plot_patch(patch, edgecolor=None, ax=None, vmin=None, vmax=None, cmap=None):
    ...
    ax.tick_params(axis='both', which='both', left='off', right='off', top='off', bottom='off', labelleft='off', labelbottom='off')
...
def plot_vst_accumulator_space(acc_space, cmap=cc.m_fire, ax=None, plot_estimates=False, plot_focus=False):
    ...
    im_plt = ax.imshow(acc_space.score, cmap=cmap, extent=(acc_space.alpha_range[0] - alpha_step0 / 2, acc_space.alpha_range[-1] + alpha_step1 / 2, acc_space.sigma_sq_range[-1] + sigma_step1 / 2, acc_space.sigma_sq_range[0] - sigma_step0 / 2))
    ax.axis('tight')
    ...
    if plot_focus:
        ...
        rect = mpatches.Rectangle((acc_space.alpha - len_a / 2, acc_space.sigma_sq - len_s / 2), len_a, len_s, ec='#0000cd', fc='none', linewidth=3)
        ax.add_artist(rect)

    if plot_estimates:
        tag_str = r'$\alpha={:.2f}$, $\beta={:.2f}$'
        bbox_props = dict(boxstyle='round', fc='w', ec='#0000cd', alpha=0.5)
        ax.annotate(tag_str.format(acc_space.alpha, acc_space.sigma_sq), xy=(acc_space.alpha, acc_space.sigma_sq), xycoords='data', xytext=(0.95, 0.05), textcoords='axes fraction', va='bottom', ha='right', color='#0000cd', size='xx-large', bbox=bbox_props, arrowprops=dict(facecolor='#0000cd', edgecolor='none', shrink=0., width=2, headwidth=3, headlength=3))