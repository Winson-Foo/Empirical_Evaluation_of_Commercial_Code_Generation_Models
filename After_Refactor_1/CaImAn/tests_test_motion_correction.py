...
import numpy as np
import caiman as cm
from caiman.motion_correction import register_translation, register_translation_3d, apply_shifts_dft, tile_and_correct, tile_and_correct_3d, motion_correct_iteration, motion_correct_iteration_fast
from scipy.ndimage import gaussian_filter
from skimage.data import lfw_subset


def gen_frame_n_templ(true_shifts: np.ndarray = np.array([2, 4])):
    """
    Generate frame and template arrays with given true shifts.
    
    Args:
        true_shifts: Array of true shifts.

    Returns:
        frame: Generated frame array.
        templ: Generated template array.
        nans: Array indicating NaN values in frame array.
    """
...
def test_register_n_apply():
    """
    Test the register and apply functions.
    """
    true_shifts = np.array([2, 4])
    frame, templ, nans = gen_frame_n_templ(true_shifts)
    shifts, src_freq, phasediff = register_translation(frame, templ)
    npt.assert_allclose(shifts, true_shifts)
    frame_cor = apply_shifts_dft(src_freq, -shifts, phasediff,
                                 is_freq=True, border_nan=True)
...
def test_tile_and_correct():
    """
    Test the tile and correct functions.
    """
    true_shifts = np.array([2, 4, 1])[:2]
    frame, templ, _ = gen_frame_n_templ(true_shifts)
    frame_cor, shifts = tile_and_correct(frame, templ, None, None, (4, 4),
                                         max_deviation_rigid=0)[:2]
    npt.assert_allclose(shifts, -true_shifts, .2)
    nans = np.isnan(frame_cor)
    npt.assert_allclose(np.corrcoef(frame_cor[~nans].ravel(),
                                    templ[~nans].ravel())[0, 1], 1, .06)


def test_motion_correct_iteration():
    """
    Test the motion correct iteration functions.
    """
    true_shifts = np.array([2, 4])
    frame, templ, nans = gen_frame_n_templ(true_shifts)
    ...
    npt.assert_allclose(shifts, -true_shifts)
    npt.assert_allclose(np.corrcoef(frame_cor[~nans].ravel(),
                                    templ[~nans].ravel())[0, 1], 1)


def gen_data(D: int = 2, noise: float = .01, T: int = 300, framerate: int = 30,
             firerate: float = 2., motion: bool = True):
    """
    Generate simulated data for motion correction.
    ...
    Args:
        ...
        firerate: Firing rate of neurons.
        motion: Whether to simulate motion.

    Returns:
        Y: Simulated data array.
        C: Calcium traces.
        S: Spike traces.
        A: Ground truth spatial footprints.
        centers: Centers of neurons.
    ...
    np.random.seed(5)
    centers = np.asarray([[np.random.randint(4, x - 4) for x in dims]
                          for _ in range(N)])
    ...
    Y = bkgrd + Yr.T.reshape((-1,) + tuple(np.array(dims) + sig_m * 4),
                             order='F')
    if motion:
...
def test_motion_correct_rigid(D: int = 2):
    """
    Test the motion correct rigid function.
