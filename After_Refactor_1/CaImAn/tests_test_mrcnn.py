...
def detect_neurons(model, img, size_range):
    results = model.detect([img], verbose=1)
    r = results[0]
    selection = np.logical_and(
        r['masks'].sum(axis=(0,1)) > size_range[0] ** 2, 
...
def mrcnn(img, size_range, weights_path):
    ...
    load_weights(model.keras_model, weights_path)
    ROIs = detect_neurons(model, img, size_range)
