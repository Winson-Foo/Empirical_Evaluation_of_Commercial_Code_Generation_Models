...
class NStepDQNAgent(BaseAgent):
    ...
    def step(self):
        ...
        loss = self.get_loss(entries.q, entries.action, entries.ret)
        self.optimizer.zero_grad()
        loss.backward()
        self.clip_gradients(config.gradient_clip)
        self.optimizer.step()

    def get_loss(self, q, action, ret):
        return 0.5 * (q.gather(1, action) - ret).pow(2).mean()

    def clip_gradients(self, gradient_clip):
