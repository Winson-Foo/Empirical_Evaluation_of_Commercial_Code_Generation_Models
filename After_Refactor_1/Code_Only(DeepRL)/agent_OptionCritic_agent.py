...
class OptionCriticAgent(BaseAgent):
    def __init__(self, config):
        ...
        self.prev_options = self._get_initial_states()
        

    def _get_initial_states(self):
        state = self.config.state_normalizer(self.task.reset())
        self.is_initial_states = tensor(np.ones((self.config.num_workers))).byte()
        return self._get_prediction(state)['q'].max(dim=-1)[1]
    ...
    @staticmethod
    def _sample_options(prediction, epsilon, prev_option, is_intial_states, worker_index):
        ...
        mask[worker_index, prev_option] = 1
    ...
    def step(self):
        ...
        for _ in range(config.rollout_length):
            prediction = self._get_prediction(self.states)
            epsilon = config.random_option_prob(config.num_workers)
            options = self._sample_options(prediction, epsilon, self.prev_options, self.is_initial_states, self.worker_index)
