...
class ErrorRate(Metric, ABC):
    def __init__(self, decoder: Decoder, target_decoder: GreedyDecoder,
                 save_output=False, dist_sync_on_step=False):
    ...
    def update(self, preds: torch.Tensor, preds_sizes: torch.Tensor,
               targets: torch.Tensor, target_sizes: torch.Tensor) -> None:
        # Unflatten targets
        split_targets = []
        offset = 0
...
class CharErrorRate(ErrorRate):
    def __init__(self, decoder: Decoder, target_decoder: GreedyDecoder,
                 save_output=False, dist_sync_on_step=False):
        super().__init__(decoder=decoder, target_decoder=target_decoder,
                         save_output=save_output, dist_sync_on_step=dist_sync_on_step)
    ...
    @staticmethod
    def cer_calc(s1: str, s2: str) -> int:
        """
...
class WordErrorRate(ErrorRate):
    ...
    @staticmethod
    def wer_calc(s1: str, s2: str) -> int:
        """
...
@torch.no_grad()
def run_evaluation(test_loader, model, decoder: Decoder, device: torch.device,
                   target_decoder: Decoder, precision: int) -> Tuple[float, float]:
    model.eval()
    ...
    for i, batch in tqdm(enumerate(test_loader), total=len(test_loader)):
        ...
        with autocast(enabled=precision == 16):
            out, output_sizes, _ = model(inputs, input_sizes)
        preds = out
        preds_sizes = output_sizes
        wer.update(preds=preds, preds_sizes=preds_sizes,
                   targets=targets, target_sizes=target_sizes)
        cer.update(preds=preds, preds_sizes=preds_sizes,
                   targets=targets, target_sizes=target_sizes)
