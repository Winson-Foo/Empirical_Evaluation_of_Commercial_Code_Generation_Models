...
from ..component import Storage, Task
...
class OptionCriticAgent(BaseAgent):
    def __init__(self, config):
        ...
        self.total_steps = 0
        self.worker_index = torch.tensor(range(config.num_workers)).long()

        self.states = self.config.state_normalizer(self.task.reset())
        self.is_initial_states = torch.ones((config.num_workers)).bool()
        self.prev_options = self.is_initial_states.clone().long()

    def sample_option(self, prediction: dict, epsilon: float, prev_option: torch.Tensor,
                      is_initial_states: torch.Tensor) -> torch.Tensor:
        """Samples new options based on predicted Q values and epsilon-greedy policy."""
        with torch.no_grad():
    ...
    def step(self) -> Tuple[float, int]:
        """Performs one episode of the environment and updates the network weights."""
        config = self.config
        ...
        for _ in range(config.rollout_length):
            ...
            next_states, rewards, terminals, info = self.task.step(actions.cpu().numpy())
        ...
        for i in reversed(range(config.rollout_length)):
            ...
            value_estimate = storage.q[i].max(dim=-1, keepdim=True)[0]
            v = value_estimate * (1 - storage.eps[i]) + storage.q[i].mean(-1).unsqueeze(-1) * storage.eps[i]
        ...
        return self.mean_episode_return(), self.num_episodes()