...
class CategoricalDQNActor(DQNActor):
    ...
    def _set_up(self):
        # Set up the categorical distribution atoms
        self.config.atoms = tensor(self.config.atoms)

    def compute_q(self, prediction):
        # Compute the Q-value distribution using the categorical distribution
...
class CategoricalDQNAgent(DQNAgent):
    def __init__(self, config):
        # Initialize the Categorical DQN agent
        BaseAgent.__init__(self, config)
        self.config = config

        # Create a lock for multi-process synchronization
        config.lock = mp.Lock()

        # Set up the categorical distribution atoms
        config.atoms = np.linspace(config.categorical_v_min, config.categorical_v_max, config.categorical_n_atoms)

        # Create the replay buffer, actor, network, and target network
        ...
        # Create the optimizer and set the actor network
    ...
    def eval_step(self, state):
        # Evaluate the agent's policy for a given state
    ...
    def compute_loss(self, transitions):
        # Compute the loss for a given batch of transitions
        config = self.config
        ...
        # Compute the target probability distribution for the next state
        with torch.no_grad():
        ...
        # Compute the target distribution and clamp it to the desired range
        ...
        # Compute the target probability using the categorical distribution
        ...
        # Compute the KL divergence between the predicted and target distributions
    ...
    def reduce_loss(self, loss):
        # Reduce the loss by taking its mean
        return loss.mean()