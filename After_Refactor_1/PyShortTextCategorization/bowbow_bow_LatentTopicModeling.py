...
import numpy as np
from shorttext.utils import textpreprocessing as tp, gensim_corpora as gc, classification_exceptions as e
...
class LatentTopicModeler(ABC):
    ...
    """
    def __init__(self, preprocessor=tp.standard_text_preprocessor_1(), normalize=True):
    ...
    def generate_corpus(self, classdict):
        ...
        :type classdict: dict
        """
        self.dictionary, self.corpus, self.class_labels = gc.generate_gensim_corpora(classdict, preprocess_and_tokenize=lambda sent: tokenize(self.preprocessor(sent)))

    @abstractmethod
    def train(self, classdict, num_topics, *args, **kwargs):
        """ Train the modeler.

        This is an abstract method of this abstract class, which raises the `NotImplementedException`.
    ...
    def __getitem__(self, shorttext):
        return self.retrieve_topic_vec(shorttext)
