...
class PPOAgent(BaseAgent):
    ...
    def step(self):
        ...
        entries.advantage = (entries.advantage - entries.advantage.mean()) / (entries.advantage.std() + 1e-8)
        ...
        for _ in range(config.optimization_epochs):
            ...
            for batch_indices in sampler:
                ...
                entry = [tensor(x[batch_indices]) for x in entries]

                prediction = self.network(entry[0], entry[1])
                ratio = (prediction['log_pi_a'] - entry[2]).exp()
                obj = ratio * entry[4]
                obj_clipped = ratio.clamp(1.0 - self.config.ppo_ratio_clip, 1.0 + self.config.ppo_ratio_clip) * entry[4]

                policy_loss = -torch.min(obj, obj_clipped).mean() - config.entropy_weight * prediction['entropy'].mean()
                value_loss = 0.5 * (entry[3] - prediction['v']).pow(2).mean()
                approx_kl = (entry[2] - prediction['log_pi_a']).mean()
