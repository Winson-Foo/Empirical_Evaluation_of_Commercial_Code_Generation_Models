...
export function getDateLocaleOrDefault(locale = 'en-US') {
  return customFormats?.[locale] ? getDateLocale(locale) : getDateLocale('en-US');
}
...
export function parseDateRange(value, locale = 'en-US') {
  ...
  const now = new Date();
  const dateLocale = getDateLocaleOrDefault(locale);
  ...
  const range = getRangeFromDateAndUnit({ now, unit, num, dateLocale });
...
export function getRangeFromDateAndUnit({
  now,
  unit,
  num,
  dateLocale = getDateLocaleOrDefault('en-US'),
}) {
  if (+num === 1) {
    switch (unit) {
      case 'day':
...
export function dateFormat(date, str, locale = 'en-US') {
  return format(date, customFormats?.[locale]?.[str] || str, {
    locale: getDateLocaleOrDefault(locale),
  });
}