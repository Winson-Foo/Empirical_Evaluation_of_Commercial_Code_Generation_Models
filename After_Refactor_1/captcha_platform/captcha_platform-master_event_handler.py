...
class FileEventHandler(FileSystemEventHandler):
    ...
    def init(self):
        model_list = self.get_model_list(self.model_conf_path)
    ...
    def get_model_list(self, model_conf_path):
        model_list = os.listdir(model_conf_path)
        return [os.path.join(model_conf_path, i) for i in model_list if i.endswith("yaml")]

    def load_models(self, model_list):
        for model in model_list:
            self.add_model(model, is_first=True)

    def add_model(self, src_path, is_first=False, count=0):
        try:
            ...
            if model_path.endswith("yaml"):
                ...
                if self.is_duplicate_size(inner_size):
                    self.logger.warning(
                        "\n-------------------------------------------------------------------\n"
                ...
                if graph_session.loaded:
                    ...
                    if count < 12 and not is_first:
                        time.sleep(5)
                        return self.add_model(src_path, is_first=is_first, count=count+1)

        except Exception as e:
            self.interface_manager.report(src_path)
            self.logger.error(e)

    def is_duplicate_size(self, size):
        for v in self.name_map.values():
            if size in v:
                return True
        return False

    def delete_model(self, src_path):
        try:
            model_path = str(src_path)
            if model_path.endswith("yaml"):
                inner_key = PathUtils.get_file_name(model_path)
