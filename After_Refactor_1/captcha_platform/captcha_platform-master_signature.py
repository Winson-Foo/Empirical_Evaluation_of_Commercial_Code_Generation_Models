...
class Signature(object):
    ...
    def __init__(self, server_type: ServerType, conf: Config):
        self.conf = conf
        self._response = Response(self.conf.response_def_map)
        self._auth = []
    ...
    def _check_req_timestamp(self, req_timestamp: int) -> bool:
        """ Check the timestamp
        @param req_timestamp: Timestamp in the request parameter (10 digits)
        """
    ...
    def _verification(self, req_params: dict) -> bool:
        ...
        try:
            ...
            raise InvalidUsage(**self._response.INVALID_PUBLIC_PARAMS)
        except Exception:
            raise InvalidUsage(**self._response.UNKNOWN_SERVER_ERROR)
        
        # NO.1 Check the timestamp
        if not self._check_req_timestamp(req_timestamp):
            raise InvalidUsage(**self._response.INVALID_TIMESTAMP)
        
        # NO.2 Check the access_id
        if not self._check_req_access_key(req_access_key):
            raise InvalidUsage(**self._response.INVALID_ACCESS_KEY)
        
        # NO.3 Check the sign
        if req_signature != self._sign(req_params):
            raise InvalidUsage(**self._response.INVALID_QUERY_STRING)
