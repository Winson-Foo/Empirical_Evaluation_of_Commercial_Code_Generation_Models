...
def split_frames(image_obj, need_frame=None):
    ...
    last_frame = image_seq[-1] if -1 in need_frame and len(need_frame) > 1 else None
    frames = [np.asarray(item) for i, item in enumerate(image_seq) if (i in need_frame or need_frame == [-1])]
    if last_frame:
        frames.append(np.asarray(last_frame))
    return frames
...
def concat_arr(img_arr):
    if len(img_arr) < 2:
        return img_arr[0]
    all_slices = np.concatenate(img_arr, axis=1)
    return all_slices
...
def blend_arr(img_arr):
    if len(img_arr) < 2:
        return img_arr[0]
    all_slices = cv2.addWeighted(img_arr[0], 0.5, img_arr[1], 0.5, 0)
    for im_slice in img_arr[2:]:
