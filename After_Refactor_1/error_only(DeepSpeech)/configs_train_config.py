...
class AugmentationConfig:
    ...
    noise_dir: Optional[str] = ''  # Directory to inject noise into audio. If default, noise Inject not added
    ...
    noise_max: float = 1.0  # Maximum noise levels to sample from. Maximum 1.0
...
class DeepSpeechConfig:
    ...
    optim: Union[SGDConfig, AdamConfig] = MISSING
    model: Optional[BiDirectionalConfig] = BiDirectionalConfig()
    ...
    def __post_init__(self):
        try:
            optim_name = self.defaults[0]["optim"]
        except IndexError:
            raise ValueError("Defaults list must contain at least one item")
        if optim_name == "sgd":
            self.optim = SGDConfig()
        ...
        try:
            model_name = self.defaults[1]["model"]
        except IndexError:
            raise ValueError("Defaults list must contain at least two items")
        if model_name == "bidirectional":
            self.model = BiDirectionalConfig()
        elif model_name == "unidirectional":
            self.model = UniDirectionalConfig()
        else:
