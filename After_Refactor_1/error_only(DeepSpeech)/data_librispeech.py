...
parser.add_argument("--target-dir", default='LibriSpeech_dataset/', type=str,
                    help="Path of the directory where the dataset will be stored.")
...
def _process_file(wav_dir, txt_dir, base_filename, root_dir):
    ...
    if not os.path.exists(full_recording_path) or not os.path.exists(root_dir):
        raise ValueError(f"Error: {full_recording_path} or {root_dir} does not exist.")
    wav_recording_path = os.path.join(wav_dir, base_filename.replace(".flac", ".wav"))
    subprocess.call(["sox", full_recording_path, "-r", str(args.sample_rate), "-b", "16", "-c", "1", wav_recording_path])
    ...
    if not os.path.exists(transcript_file):
        raise ValueError(f"Error: transcript file {transcript_file} does not exist.")
    ...
    with open(txt_transcript_path, "w") as f:
        ...
        if key not in transcriptions:
            raise ValueError(f"Error: key {key} is not in the transcriptions")
    ...
    for split_type, lst_libri_urls in LIBRI_SPEECH_URLS.items():
        ...
        for url in lst_libri_urls:
            ...
            if not os.path.exists(extracted_dir):
                raise ValueError(f"Error: archive {filename} was not properly uncompressed.")
