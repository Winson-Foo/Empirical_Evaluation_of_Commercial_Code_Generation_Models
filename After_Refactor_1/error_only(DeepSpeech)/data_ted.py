        ...
        for stm_line in f:
            try:
                ...
                transcript = unicodedata.normalize("NFKD",
                                                   " ".join(t for t in tokens[6:]).strip()).encode("utf-8",
                                                                                                       "ignore").decode(
                    "utf-8", "ignore")
                ...
                print(f"An error occurred in get_utterances_from_stm: {e}")
    return res
...
def cut_utterance(src_sph_file, target_wav_file, start_time, end_time, sample_rate=16000):
    try:
        ...
        print(f"An error occurred in cut_utterance: {e}")
...
def filter_short_utterances(utterance_info, min_len_sec=1.0):
    try:
        ...
        print(f"An error occurred in filter_short_utterances: {e}")
        return False
...
def prepare_dir(ted_dir):
    try:
        ...
        for sph_file in tqdm(entries, total=len(entries)):
            ...
            if os.path.exists(sph_file_full) and os.path.exists(stm_file_full):
                all_utterances = get_utterances_from_stm(stm_file_full)

                all_utterances = list(filter(filter_short_utterances, all_utterances))
                ...
                print(f"Skipping {sph_file_full} and {stm_file_full} - does not exist")
    except Exception as e:
        print(f"An error occurred in prepare_dir: {e}")
