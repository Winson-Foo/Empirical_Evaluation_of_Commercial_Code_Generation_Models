...
def rescale_tensorflow(
    ...
    img_resized = tf.image.resize(x.raw, size=target_shape[1:-1])
...
def rescale_pytorch(x: ep.PyTorchTensor, target_shape: List[int]) -> ep.PyTorchTensor:
    img_resized = torch.nn.functional.interpolate(
        x.raw, size=target_shape[2:], mode="bilinear", align_corners=False
    )
...
def rescale_images(
    ...
    rescale_funcs = {
        ep.PyTorchTensor: rescale_pytorch,
        ep.TensorFlowTensor: rescale_tensorflow,
        ep.NumPyTensor: rescale_numpy,
        ep.JAXTensor: rescale_jax,
    }
    ...
    x = rescale_funcs[type(x)](x, target_shape)
