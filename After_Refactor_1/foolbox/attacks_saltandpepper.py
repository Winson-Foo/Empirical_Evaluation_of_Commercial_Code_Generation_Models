...
from .base import MinimizationAttack, T, get_criterion, get_is_adversarial, get_channel_axis, raise_if_kwargs, verify_input_bounds
...
class SaltAndPepperNoiseAttack(MinimizationAttack):
    ...
    def run(
        ...
        result = x0
        best_advs_norms = ep.where(is_adversarial(result), ep.zeros(x0, N), ep.full(x0, N, ep.inf))
