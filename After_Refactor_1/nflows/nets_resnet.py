...
class ResidualBlock(nn.Module):
    ...
    def forward(self, inputs, context=None):
        temps = self._apply_batch_norm(inputs)
        temps = self.activation(temps)
        temps = self._apply_linear(temps)
        temps = self._apply_batch_norm(temps)
        temps = self.activation(temps)
        temps = self.dropout(temps)
        temps = self._apply_linear(temps)
        temps = self._apply_context(temps, context)
        return inputs + temps

    def _apply_batch_norm(self, inputs):
        if self.use_batch_norm:
            return self.batch_norm_layers[0](inputs)
        return inputs

    def _apply_linear(self, inputs):
        return self.linear_layers[0](inputs)

    def _apply_context(self, inputs, context):
        if context is not None:
            return F.glu(
                torch.cat((inputs, self.context_layer(context)), dim=1), dim=1
            )
        return inputs
...
class ResidualNet(nn.Module):
    ...
    def forward(self, inputs, context=None):
        temps = self.initial_layer(torch.cat((inputs, context), dim=1)) if context is not None else self.initial_layer(inputs)
...
class ConvResidualBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.activation = activation
        self.context_layer = self._initialize_context(context_channels) if context_channels is not None else None
        self.use_batch_norm = use_batch_norm
        if use_batch_norm:
            self.batch_norm_layers = nn.ModuleList(
    ...
    def _initialize_context(self, context_channels):
        return nn.Conv2d(
    ...
    def _apply_conv(self, inputs):
        return self.conv_layers[0](inputs)
...
class ConvResidualNet(nn.Module):
    def __init__(
        ...
        self.initial_layer = nn.Conv2d(
            in_channels=in_channels + context_channels if context_channels is not None else in_channels,
