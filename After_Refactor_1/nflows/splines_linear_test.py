...
class LinearSplineTest(torchtestcase.TorchTestCase):
    def test_forward_inverse_are_consistent(self):
        ...
        def call_linear_spline(inputs, inverse=False):
            return splines.linear_spline(
                inputs=inputs, unnormalized_pdf=unnormalized_pdf, inverse=inverse
            )

        inputs = torch.rand(*shape)
        outputs, logabsdet = call_linear_spline(inputs, inverse=False)
        inputs_inv, logabsdet_inv = call_linear_spline(outputs, inverse=True)
...
class UnconstrainedLinearSplineTest(torchtestcase.TorchTestCase):
    def test_forward_inverse_are_consistent(self):
        ...
        def call_unconstrained_linear_spline(inputs, inverse=False):
            return splines.unconstrained_linear_spline(
                inputs=inputs, unnormalized_pdf=unnormalized_pdf, inverse=inverse
            )

        inputs = 3 * torch.randn(*shape)  # Note inputs are outside [0,1].
        outputs, logabsdet = call_unconstrained_linear_spline(inputs, inverse=False)
        inputs_inv, logabsdet_inv = call_unconstrained_linear_spline(outputs, inverse=True)
