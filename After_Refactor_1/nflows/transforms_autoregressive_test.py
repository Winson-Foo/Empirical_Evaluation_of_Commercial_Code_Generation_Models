...
class MaskedAffineAutoregressiveTransformTest(TransformTest):
        ...
        for use_residual_blocks, random_mask in [
            ...
            with self.subTest(
                ...
                self._test_transform(
                    autoregressive.MaskedAffineAutoregressiveTransform,
                    features,
                    inputs,
                    use_residual_blocks,
                    random_mask,
    ...
    def _test_transform(self, transform_class, features, inputs, use_residual_blocks, random_mask):
        ...
        self.assert_tensor_is_good(outputs, [inputs.shape[0], features])
        self.assert_tensor_is_good(logabsdet, [inputs.shape[0]])

    def _test_inverse(self, transform_class, features, inputs, use_residual_blocks, random_mask):
        transform = transform_class(
            features=features,
            hidden_features=30,
            num_blocks=5,
    ...
    def _test_forward_inverse_consistency(self, transform_class, features, inputs, use_residual_blocks, random_mask):
        transform = transform_class(
            features=features,
            hidden_features=30,
            num_blocks=5,
