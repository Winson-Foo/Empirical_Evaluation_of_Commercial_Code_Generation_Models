...
class MaskedLinear(nn.Linear):
    ...
    def __init__(self, in_degrees, out_features, autoregressive_features, random_mask, is_output, bias=True):
        ...
        mask, degrees = self._get_mask_and_degrees(in_degrees, out_features, autoregressive_features, random_mask, is_output)
...
class MaskedFeedforwardBlock(nn.Module):
    ...
    def forward(self, inputs, context=None):
        temps = self.batch_norm(inputs) if self.batch_norm else inputs
...
class MaskedResidualBlock(nn.Module):
    ...
    def forward(self, inputs, context=None):
        temps = self.batch_norm_layers[0](inputs) if self.use_batch_norm else inputs
        temps = self.activation(temps)
        temps = self.linear_layers[0](temps)
        if context is not None:
            temps += self.context_layer(context)
        temps = self.batch_norm_layers[1](temps) if self.use_batch_norm else temps
        temps = self.activation(temps)
...
class MADE(nn.Module):
    ...
    def __init__(self, features, hidden_features, context_features=None, num_blocks=2, output_multiplier=1, use_residual_blocks=True, random_mask=False, activation=F.relu, dropout_probability=0.0, use_batch_norm=False):
        ...
        block_constructor = MaskedResidualBlock if use_residual_blocks else MaskedFeedforwardBlock
