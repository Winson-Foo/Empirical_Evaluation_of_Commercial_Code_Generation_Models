...
class HouseholderSequence(Transform):
    ...
    def __init__(self, features, num_transforms):
        ...
        self.q_vectors = self._create_q_vectors()
        
    def _create_q_vectors(self):
        import numpy as np
        ...
        qv = tile(torch.eye(self.num_transforms // 2, self.features), 0, 2)

        if np.mod(self.num_transforms, 2) != 0:  # odd number of transforms, including 1
            qv = torch.cat((qv, torch.zeros(1, self.features)))
            qv[-1, self.num_transforms // 2] = 1
        return nn.Parameter(qv)

    @staticmethod
    def _apply_transform(inputs, q_vectors):
    ...
    def forward(self, inputs, context=None):
        return self._apply_transform(inputs, self.q_vectors)
    ...
    def inverse(self, inputs, context=None):
        ...
        return self._apply_transform(inputs, self.q_vectors[reverse_idx])
