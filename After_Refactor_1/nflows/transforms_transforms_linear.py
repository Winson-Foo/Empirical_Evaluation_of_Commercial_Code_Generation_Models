...
class NaiveLinear(Transform):
    ...
    def __init__(self, features: int, orthogonal_initialization: bool = True, using_cache: bool = False):
        """
        Constructor.

        Args:
            features: int, number of input features.
            orthogonal_initialization: bool, if True initialize weights to be a random orthogonal matrix.
        """
        super().__init__()
        if features < 1:
    ...
    def logabsdet(self):
        ...
        where:
            D = num of features
        """
        return torchutils.logabsdet(self.weight)