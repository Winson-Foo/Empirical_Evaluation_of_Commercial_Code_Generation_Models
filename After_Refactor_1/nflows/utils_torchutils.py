...
def tile(x: torch.Tensor, n: int) -> torch.Tensor:
    ...
    x_ = x.reshape(-1).repeat(n)
    x_ = x_.reshape(n, -1).transpose(1, 0).reshape(-1)
    return x_


def sum_except_batch(x: torch.Tensor, num_batch_dims: int = 1) -> torch.Tensor:
    if num_batch_dims < 0:
        raise ValueError("Number of batch dimensions must be a non-negative integer.")
...
def merge_leading_dims(x: torch.Tensor, num_dims: int) -> torch.Tensor:
    if num_dims <= 0:
        raise ValueError("Number of leading dims must be a positive integer.")
    if num_dims > x.dim():
