...
class OptionCriticAgent(BaseAgent):
    ...
    def sample_option(self, prediction, epsilon, prev_option, is_init_states):
        with torch.no_grad():
            ...
            options = torch.where(is_init_states, options, options_hat)
        return options

    def step(self):
        config = self.config
        storage = Storage(config.rollout_length, ['beta', 'option', 'beta_advantage', 'prev_option', 'init_state', 'eps'])
        total_steps_div_workers = self.total_steps // config.num_workers
        ...
        for _ in range(config.rollout_length):
            ...
            if total_steps_div_workers % config.target_network_update_freq == 0:
                self.target_network.load_state_dict(self.network.state_dict())
            total_steps_div_workers = self.total_steps // config.num_workers
