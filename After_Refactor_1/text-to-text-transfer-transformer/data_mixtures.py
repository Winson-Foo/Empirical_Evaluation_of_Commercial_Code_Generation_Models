...
_SUPER_GLUE_WEIGHT_MAPPING_SENTINEL = t5.data.glue_utils.get_super_glue_weight_mapping_sentinel()
...
MixtureRegistry.add(
    ...
    [task for task in _glue_tasks if "mnli" in task],
    default_rate=1.0
)
...
MixtureRegistry.add(
    "all_proportional",
    [(task, _dedupe(task)) for task in _supervised_tasks + ["c4_v020_unsupervised"]]
)

MixtureRegistry.add(
    "all_mix",
    ([("c4_v020_unsupervised", t5.data.rate_unsupervised)] +
     [(task, _dedupe(task)) for task in _supervised_tasks])
)
