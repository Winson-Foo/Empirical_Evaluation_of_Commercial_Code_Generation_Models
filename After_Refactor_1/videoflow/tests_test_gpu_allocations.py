...
from videoflow.core.node import Node, TaskModuleNode, ProcessorNode
...
def test_nb_tasks_created():
    ...
    tasks_data = Node.get_tasks_data(tsort)

    ee = RealtimeExecutionEngine()
    ee.create_processes(tasks_data)
    assert len(tsort) == len(ee.procs)
...
def test_nb_tasks_created_1():
    ...
    ee = RealtimeExecutionEngine()
    ee.create_processes(tasks_data)
    assert len(ee.procs) == 3

class IdentityProcessorGpuOnly(ProcessorNode):
    def __init__(self, fps=-1):
        super().__init__(device_type=GPU)
        self._wts = max(0, 1.0 / fps) if fps > 0 else 0 # wait time in seconds
...
def test_gpu_nodes_accepted(monkeypatch):
    ...
    ee1.create_processes(tasks_data)
