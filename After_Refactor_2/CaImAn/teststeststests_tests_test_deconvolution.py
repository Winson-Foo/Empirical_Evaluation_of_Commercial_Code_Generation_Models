...
def analyze_data(method, p, g, sn):
    t = time()
    for i, sn_value in enumerate(sn):  # high and low SNR
        y, c, s = [a[0] for a in generate_data(g, sn_value)]
        res = constrained_foopsi(y, g=g, sn=sn_value, p=p, method=method)
...
def test_oasis():
    analyze_data('oasis', 1, [.95], [.2, .5])
    analyze_data('oasis', 2, [1.7, -.71], [.2, .5])