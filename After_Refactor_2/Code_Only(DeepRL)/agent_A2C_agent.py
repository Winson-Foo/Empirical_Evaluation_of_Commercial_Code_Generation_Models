...
class A2CAgent(BaseAgent):
    def __init__(self, config):
        super().__init__(config) # using super to inherit from BaseAgent
    ...
    def step(self):
        ...
        loss = (policy_loss - config.entropy_weight * entropy_loss + config.value_loss_weight * value_loss) # combining calculation of loss
        loss.backward()
