...
class QuantileRegressionDQNAgent(DQNAgent):
    ...
    def _initialize(self):
        ...
        self.actor = QuantileRegressionDQNActor(self.config)
        ...
        self.cumulative_density = tensor((2 * np.arange(self.config.num_quantiles) + 1) / (2.0 * self.config.num_quantiles)).view(1, -1)
    ...
    def compute_loss(self, transitions):
        ...
        negative_mask = (diff.detach() < 0).float()

        loss = huber(diff) * (self.cumulative_density - negative_mask).abs()
