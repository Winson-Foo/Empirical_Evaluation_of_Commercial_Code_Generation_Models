...
const APIKeyDialog = ({ show, dialogProps, onCancel, onConfirm }) => {
  ...
  const handleCopyClick = async (event) => {
    await navigator.clipboard.writeText(dialogProps.key.apiKey);
  ...
  const component = show && (
    <Dialog fullWidth maxWidth='sm' open={show} onClose={onCancel} aria-labelledby='alert-dialog-title' aria-describedby='alert-dialog-description'>
        ...
        {dialogProps.type === 'EDIT' && (
          <Box sx={{ p: 2 }}>
            ...
            <Stack direction='row' sx={{ mb: 1 }}>
              ...
              <IconButton title='Copy API Key' color='success' onClick={handleCopyClick}>
                <IconCopy />
              </IconButton>
              <Popover
                open={openPopOver}
