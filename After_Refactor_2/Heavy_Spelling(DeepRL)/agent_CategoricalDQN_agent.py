...
class CategoricalDQNActor(DQNActor):
    ...
    def calculate_q_values(self, prediction):
...
class CategoricalDQNAgent(DQNAgent):
    ...
    def evaluate(self, state):
        ...
        q = self.actor.calculate_q_values(prediction)
        action = to_np(q.argmax(-1))
        self.config.state_normalizer.unset_read_only()
        return action

    def calculate_loss(self, transitions):
        config = self.config
        states = self.config.state_normalizer(transitions.state)
        next_states = self.config.state_normalizer(transitions.next_state)
        with torch.no_grad():
            prob_next = self.target_network(next_states)['prob']
            q_next = self.actor.calculate_q_values(prob_next)
            if config.double_q:
                a_next = torch.argmax(self.actor.calculate_q_values(self.network(next_states)['prob']), dim=-1)
            else:
