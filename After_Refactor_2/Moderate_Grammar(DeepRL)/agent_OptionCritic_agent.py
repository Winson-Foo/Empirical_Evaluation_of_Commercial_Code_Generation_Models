...
class OptionCriticAgent(BaseAgent):
    ...
    def _sample_option(self, prediction: dict, epsilon: float, prev_option: torch.tensor, is_initial_states: torch.tensor) -> torch.tensor:
        """
        Sample the current option using epsilon-greedy exploration mechanism.

        Args:
            prediction (dict): dictionary containing the predicted values.
            epsilon (float): exploration probability.
            prev_option (torch.tensor): the previous option taken by each worker.
            is_intial_states (torch.tensor): boolean tensor indicating whether the current state is initial or not.

        Returns:
            torch.tensor: tensor containing the selected options for each worker.
        """
        with torch.no_grad():
    ...
    def _step(self) -> Tuple[torch.tensor, float]:
        """
        Run a single step of the agent.

        Returns:
            Tuple[torch.tensor, float]: a tuple containing the tensor of predicted actions and the entropy of the action distribution.
        """
        config = self.config
        ...
        for _ in range(config.rollout_length):
            ...
            options = self._sample_option(prediction, epsilon, self.prev_options, self.is_initial_states)
