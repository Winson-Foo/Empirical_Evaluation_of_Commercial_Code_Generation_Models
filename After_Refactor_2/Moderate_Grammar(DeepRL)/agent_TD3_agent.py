...
class TD3Agent(BaseAgent):
    def __init__(self, config):
        ...
        self.total_steps = MAGIC_ZERO
        self.state = None

    def soft_update(self, target, src):
        for target_param, param in zip(target.parameters(), src.parameters()):
            target_param.detach_()
            target_param.copy_(target_param * (MAGIC_ONE - self.config.target_network_mix) +
    ...
    def step(self):
        ...
        self.replay.feed(dict(
            ...
            mask=MAGIC_ONE - np.asarray(done, dtype=np.int32),
        ))

        if done[0]:
            self.random_process.reset_states()
        self.state = next_state
        self.total_steps += MAGIC_ONE
        ...
        if self.total_steps >= config.warm_up:
            ...
            if self.total_steps % config.td3_delay == MAGIC_ZERO:
                action = self.network(states)
                policy_loss = -self.network.q(states, action)[MAGIC_ZERO].mean()
