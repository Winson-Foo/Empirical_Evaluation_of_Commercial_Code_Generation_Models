...
def get_word_embeddings(tokens, wvmodel):
    """Create word embeddings for given tokens using pre-trained word-embedding models."""
    embeddings = {token: wvmodel[token] for token in tokens}
    return embeddings


def collapsed_idx_func(i, j, nb_tokens_second_sent):
    """Compute the collapsed index of two tokens."""
    return i * nb_tokens_second_sent + j


def get_T(first_sent_tokens, second_sent_tokens, wordvecs, distancefunc):
    """Compute the matrix T for first and second sentence."""
    ...
    for i, j in product(range(nb_tokens_first_sent), range(nb_tokens_second_sent)):
        T[collapsed_idx_func(i, j, nb_tokens_second_sent)] = distancefunc(wordvecs[first_sent_tokens[i]],
                                                                          wordvecs[second_sent_tokens[j]])
    return T


def get_Aeq(first_sent_tokens, second_sent_tokens, nb_tokens_first_sent, nb_tokens_second_sent, buckets):
    """Compute the matrix Aeq for first and second sentence."""
    Aeq = csr_matrix((nb_tokens_first_sent + nb_tokens_second_sent,
    ...
    for i in range(nb_tokens_first_sent):
        for j in range(nb_tokens_second_sent):
            Aeq[i, collapsed_idx_func(i, j, nb_tokens_second_sent)] = 1.
        beq[i] = buckets[first_sent_tokens[i]]
    for j in range(nb_tokens_second_sent):
        for i in range(nb_tokens_first_sent):
            Aeq[j + nb_tokens_first_sent, collapsed_idx_func(i, j, nb_tokens_second_sent)] = 1.
        beq[j + nb_tokens_first_sent] = buckets[second_sent_tokens[j]]
...
def word_mover_distance_linprog(first_sent_tokens, second_sent_tokens, wvmodel, distancefunc=euclidean):
    ...
    wordvecs = get_word_embeddings(all_tokens, wvmodel)

    first_sent_buckets = tokens_to_fracdict(first_sent_tokens)
    second_sent_buckets = tokens_to_fracdict(second_sent_tokens)

    T = get_T(first_sent_tokens, second_sent_tokens, wordvecs, distancefunc)

    Aeq, beq = get_Aeq(first_sent_tokens, second_sent_tokens, nb_tokens_first_sent, nb_tokens_second_sent,
                       dict(first_sent_buckets, **second_sent_buckets))
