...
class OptionCriticAgent(BaseAgent):
    ...
    def sample_option(self, prediction, epsilon, prev_option, is_initial_states):
        """
        Function to sample option for each worker using the provided prediction.

        Args:
        - prediction: dictionary containing the predicted Q values, pi values, and betas.
        - epsilon: probability of selecting a random option.
        - prev_option: tensor of previously selected options per worker.
        - is_initial_states: tensor indicating whether the worker state is the initial state.

        Returns:
        - options: tensor of options selected for each worker.
        """
        with torch.no_grad():
    ...
    def update_storage(self, config, storage, prediction, actions, rewards, terminals, info, next_states, entropy):
        """
        Function to update the Storage object with the values predicted by the network.

        Args:
        - config: configuration object.
        - storage: Storage object containing rollout data.
        - prediction: dictionary containing the predicted Q values, pi values, and betas.
        - actions: tensor of actions selected for each worker.
        - rewards: tensor of rewards received by each worker.
        - terminals: tensor indicating whether the worker has reached a terminal state.
        - info: dictionary containing additional information about each worker.
        - next_states: tensor of states observed after taking actions.
        - entropy: tensor of entropy for each worker's distribution.
        """
    ...
    def calculate_returns(self, config, storage):
        """
        Function to calculate the returns for each rollout step.
    ...
    def calculate_losses(self, config, storage, entries):
        """
        Function to calculate the loss values for each batch of rollout data.

        Args:
        - config: configuration object.
        - storage: Storage object containing rollout data.
        - entries: dictionary containing the q values, beta values, log_pi values, etc.

        Returns:
        - tuple containing the pi loss value, q loss value, and beta loss value.
        """
    ...
    def optimize_network(self, config, loss):
        """
        Function to optimize the network using the calculated loss values.

        Args:
        - config: configuration object.
        - loss: tuple containing the pi loss value, q loss value, and beta loss value.
    ...
    def step(self):
        """
        Function to take a training step for each rollout.
        ...
        for _ in range(config.rollout_length):
            ...
            self.update_storage(config, storage, prediction, actions, rewards, terminals, info, next_states, entropy)

        entries = storage.extract(
            ['q', 'beta', 'log_pi', 'ret', 'advantage', 'beta_advantage', 'entropy', 'option', 'action', 'init_state', 'prev_option'])

        self.calculate_returns(config, storage)

        loss = self.calculate_losses(config, storage, entries)

        self.optimize_network(config, loss)