...
threading.Thread(target=lambda: event_loop(system_config, system_config.model_path, interface_manager)).start()
...
def common_request(request):
    ...
    if not bytes_batch:
        logger.error('Type[{}] - Site[{}] - Response[{}] - {} ms'.format(
            request.json['model_type'], request.json['model_site'], response, (time.time() - start_time) * 1000))
    ...
    if not image_batch:
        logger.error('[{}] - Size[{}] - Name[{}] - Response[{}] - {} ms'.format(
            interface.name, size_string, request.json.get('model_name'), response, (time.time() - start_time) * 1000))
        return json(response)
