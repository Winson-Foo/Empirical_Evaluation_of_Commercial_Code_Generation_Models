...
def calculate_coordinates(param, is_range=True, is_integer=True):
    result_group = []
...
def parse_multi_img(image_bytes, param_group):
    ...
    for param in param_group:
        coord_ranges = calculate_coordinates(param, True, True)
        for coord_range in coord_ranges:
            corp_arr = image_arr[coord_range[0][0]: coord_range[0][1], coord_range[1][0]: coord_range[1][1]]
...
def get_coordinate(label: str, param_group, title_index=None):
    if title_index is None:
        title_index = [0]
    param = param_group[-1]
    coord_map = calculate_coordinates(param, is_range=False, is_integer=True)
