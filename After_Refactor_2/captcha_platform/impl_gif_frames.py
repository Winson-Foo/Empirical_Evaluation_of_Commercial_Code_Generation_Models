...
def split_frames(image_obj: bytes, need_frame: List[int] = None) -> List[np.ndarray]:
...
def concat_frames(image_obj: bytes, need_frame: List[int] = None) -> np.ndarray:
    if not need_frame:
        need_frame = [0]
...
def blend_frame(image_obj: bytes, need_frame: List[int] = None) -> np.ndarray:
    if not need_frame:
        need_frame = [-1]
    img_arr = split_frames(image_obj, need_frame)
...
def all_frames(image_obj: bytes) -> List[bytes]:
...
def get_continuity_max(src: List[int]) -> str:
    if not src:
        return ""
