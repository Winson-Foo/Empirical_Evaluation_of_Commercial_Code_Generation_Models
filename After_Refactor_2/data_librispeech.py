...
from deepspeech_pytorch.data import LIBRI_SPEECH_URLS
...
def preprocess_transcript(phrase):
    """Preprocesses the transcript."""
    return phrase.strip().upper()


def process_file(wav_dir, txt_dir, base_filename, root_dir, args):
    ...
    with open(txt_transcript_path, "w") as f:
        ...
        f.write(preprocess_transcript(transcriptions[key]))
        f.flush()


def download_file(target_dir, files_to_use, split_type, lst_libri_urls):
    """Download, extract, and process files from the dataset."""
    ...
    for url in lst_libri_urls:
        ...
        for root, subdirs, files in tqdm(os.walk(extracted_dir)):
            for f in files:
                if f.find(".flac") != -1:
                    process_file(wav_dir=split_wav_dir, txt_dir=split_txt_dir,
                                  base_filename=f, root_dir=root, args=args)
        
        print("Finished {}".format(url))
        shutil.rmtree(extracted_dir)
        
        # Prune to min/max duration
        if split_type == 'train':
            create_manifest(
                data_path=split_dir,
...
def main():
    ...
    for split_type, lst_libri_urls in LIBRI_SPEECH_URLS.items():
        download_file(target_dl_dir, files_to_dl, split_type, lst_libri_urls)
