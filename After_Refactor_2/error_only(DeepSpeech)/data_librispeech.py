...
def _process_file(wav_dir, txt_dir, base_filename, root_dir):
    ...
    if not os.path.exists(full_recording_path) and not os.path.exists(root_dir):
        print(f"{full_recording_path} or {root_dir} does not exist")
        return False
    ...
    with open(txt_transcript_path, "w") as f:
        ...
        if key not in transcriptions:
            print(f"{key} is not in the transcriptions")
            return False
    ...
    for split_type, lst_libri_urls in LIBRI_SPEECH_URLS.items():
        ...
        for url in lst_libri_urls:
            ...
            if not os.path.exists(extracted_dir):
                print(f"Archive {filename} was not properly uncompressed.")
                continue
            failed_process = []
            for root, subdirs, files in tqdm(os.walk(extracted_dir)):
                for f in files:
                    if f.find(".flac") != -1:
                        if not _process_file(wav_dir=split_wav_dir, txt_dir=split_txt_dir,
                                      base_filename=f, root_dir=root):
                            failed_process.append(f)
            if failed_process:
                print(f"Failed to process the following files: {failed_process}")
            else:
