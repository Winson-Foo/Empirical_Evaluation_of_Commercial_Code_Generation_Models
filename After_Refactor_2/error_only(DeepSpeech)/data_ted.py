    ...
    with io.open(stm_file, "r", encoding='utf-8') as f:
        for stm_line in f:
            try:
                ...
                print('Error in line:', stm_line)
    return res


def cut_utterance(src_sph_file, target_wav_file, start_time, end_time, sample_rate=16000):
    try:
        subprocess.call(f"sox {src_sph_file}  -r {str(sample_rate)} -b 16 -c 1 {target_wav_file} trim {start_time} ={end_time}", 
                        shell=True)
    except:
        print('Error during audio file processing:', src_sph_file)
...
def prepare_dir(ted_dir):
    ...
    for sph_file in tqdm(entries, total=len(entries)):
        try:
            ...
            print('Error processing file:', sph_file)
