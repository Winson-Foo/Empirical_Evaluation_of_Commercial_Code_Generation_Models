...
def transcribe_file():
    if request.method == 'POST':
        ...
        if 'file' not in request.files:
            res['status'] = "error"
            res['message'] = "No audio file passed for transcription"
            return jsonify(res)
        try:
            ...
            if file_extension.lower() not in ALLOWED_EXTENSIONS:
                res['status'] = "error"
                res['message'] = "{} is not a supported format".format(file_extension)
                return jsonify(res)
            ...
            res['status'] = "error"
            res['message'] = "Transcription failed: {}".format(str(e))
        return jsonify(res)
...
def main(cfg: ServerConfig):
    ...
    try:
        ...
        logging.error("Failed to start the server: {}".format(str(e)))
