...
from .base import FixedEpsilonAttack, T, get_channel_axis, raise_if_kwargs, verify_input_bounds
...
class GenAttack(FixedEpsilonAttack):
    ...
    def __init__(self, steps: int = 1000, population: int = 10,
                 mutation_probability: float = 0.10, mutation_range: float = 0.15,
    ...
    def apply_noise(self, x: ep.TensorType, noise: ep.TensorType, epsilon: float,
                    channel_axis: Optional[int]) -> ep.TensorType:
    ...
    def choice(self, a: int, size: Union[int, ep.TensorType],
               replace: bool, p: ep.TensorType) -> Any:
        p = p.numpy()
        x = ep.numpy.random.choice(a, size, replace, p)
        return x
