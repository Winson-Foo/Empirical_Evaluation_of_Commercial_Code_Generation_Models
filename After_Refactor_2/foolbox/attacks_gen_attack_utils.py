...
def rescale_jax(x: ep.JAXTensor, target_shape: List[int]) -> ep.JAXTensor:
    ...
    def interpolate_bilinear(im: jnp.ndarray, rows: jnp.ndarray, cols: jnp.ndarray) -> jnp.ndarray:
        col_lo = jnp.floor(cols).astype(int)
        col_hi = col_lo + 1
        row_lo = jnp.floor(rows).astype(int)
        row_hi = row_lo + 1

        def cclip(cols: jnp.ndarray) -> jnp.ndarray:
            return jnp.clip(cols, 0, ncols - 1)

        def rclip(rows: jnp.ndarray) -> jnp.ndarray:
            return jnp.clip(rows, 0, nrows - 1)
        ...
        wa = jnp.expand_dims((col_hi - cols) * (row_hi - rows), -1)
        wb = jnp.expand_dims((col_hi - cols) * (rows - row_lo), -1)
        wc = jnp.expand_dims((cols - col_lo) * (row_hi - rows), -1)
        wd = jnp.expand_dims((cols - col_lo) * (rows - row_lo), -1)
    ...
    rows = jnp.linspace(deltas[0], nrows - deltas[0], np.int32(resize_rates[0] * nrows))
    cols = jnp.linspace(deltas[1], ncols - deltas[1], np.int32(resize_rates[1] * ncols))
    rows_grid, cols_grid = jnp.meshgrid(rows - 0.5, cols - 0.5, indexing="ij")

    img_resize_vec = interpolate_bilinear(img, rows_grid.flatten(), cols_grid.flatten())
    img_resize = jnp.reshape(
...
def rescale_numpy(x: ep.NumPyTensor, target_shape: List[int]) -> ep.NumPyTensor:
    ...
    def interpolate_bilinear(im: np.ndarray, rows: np.ndarray, cols: np.ndarray) -> np.ndarray:
...
def rescale_tensorflow(
    ...
    img_resized = tf.image.resize(img, size=target_shape[1:-1])
...
def rescale_images(
    ...
    if isinstance(x, ep.PyTorchTensor):
        ...
        x = rescale_pytorch(x, target_shape)
        ...
        x = rescale_tensorflow(x, target_shape)
        ...
        x = rescale_numpy(x, target_shape)
        if channel_axis != x.ndim - 1:
            x = swap_axes(x, channel_axis, x.ndim - 1)
