...
def test_rescale_axis(request: Any, dummy: ep.Tensor) -> None:
    ...
    x_up_np_ep = _rescale_images(x_np_ep, (16, 3, 128, 128), 1)
    x_up_np = x_up_np_ep.numpy()

    x = ep.from_numpy(dummy, x_np)
    x_ep = ep.astensor(x)
    x_up_ep = _rescale_images(x_ep, (16, 3, 128, 128), 1)
    x_up = x_up_ep.numpy()
...
def test_rescale_axis_nhwc(request: Any, dummy: ep.Tensor) -> None:
    ...
    x_np_ep = ep.astensor(x_np)
    x_up_np_ep = _rescale_images(x_np_ep, (16, 128, 128, 3), -1)
    x_up_np = x_up_np_ep.numpy()

    x = ep.from_numpy(dummy, x_np)
    x_ep = ep.astensor(x)
    x_up_ep = _rescale_images(x_ep, (16, 128, 128, 3), -1)
    x_up = x_up_ep.numpy()

    assert np.allclose(x_up_np, x_up, atol=1e-5)


def _rescale_images(x: ep.Tensor, shape: tuple, axis: int) -> ep.Tensor:
    return rescale_images(x, shape, axis)