...
from models import Model  # assuming models is the correct module path

from git_cloner import clone
from model_loader import ModelLoader


def clone_git_repository(url: str, overwrite: bool) -> str:
    """Clones a Git repository to the local machine.

    Args:
        url: URL to the Git repository.
        overwrite: Whether to overwrite an existing local repository.

    Returns:
        The local path of the cloned repository.
    """
    return clone(url, overwrite=overwrite)


def get_model(
    url: str, module_name: str = "foolbox_model", overwrite: bool = False, **kwargs: Optional[Any]
) -> Model:
    ...
    Args:
        ...
        overwrite: Whether to overwrite an existing local repository.
        kwargs: Optional set of parameters that will be used by the instantiated model.

    Returns:
        A Foolbox-wrapped model instance.
    """
    repo_path = clone_git_repository(url, overwrite=overwrite)
    model_loader = ModelLoader.get()
    model = model_loader.load(repo_path, module_name=module_name, **kwargs)
    return model