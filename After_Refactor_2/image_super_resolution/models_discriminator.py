...
class Discriminator:
    ...
    Args:
        patch_size (int): Determines input size as (patch_size, patch_size, 3).
        kernel_size (int, optional): Size of the kernel in the conv blocks.

    Attributes:
        model (keras.model): Keras model.
        name (str): Name used to identify what discriminator is used during GANs training.
        block_param (dict): Dictionary that determines the number of filters and the strides for each conv block.
    ...
    def __init__(self, patch_size, kernel_size=3):
        ...
        self.block_param = dict(filters=(64, 128, 128, 256, 256, 512, 512), strides=(2, 1, 2, 1, 1, 1, 1))
    ...
    def _conv_block(self, inputs, **kwargs):
