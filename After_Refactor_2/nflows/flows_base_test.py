...
class FlowTest(torchtestcase.TorchTestCase):
    def setUp(self):
        ...
        self.maybe_context = torch.randn(self.context_size, *self.context_shape)
        self.context = torch.randn(self.context_size, *self.context_shape)

    def test_log_prob(self):
        for context in [None, self.maybe_context]:
            with self.subTest(context=context):
                log_prob = self.flow.log_prob(self.inputs, context=context)
    ...
    def test_sample_and_log_prob_with_context(self):
        samples, log_prob = self.flow.sample_and_log_prob(self.num_samples, context=self.context)
        self.assertIsInstance(samples, torch.Tensor)
        self.assertIsInstance(log_prob, torch.Tensor)
        self.assertEqual(
    ...
    def test_transform_to_noise(self):
        for context in [None, self.maybe_context]:
            with self.subTest(context=context):
                noise = self.flow.transform_to_noise(self.inputs, context=context)
