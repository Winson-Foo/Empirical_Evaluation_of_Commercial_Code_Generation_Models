...
class MultiscaleCompositeTransform(Transform):
    ...
    def add_transform(self, transform, transform_output_shape):
        ...
        Parameters:
            ...
            transform_output_shape: tuple, shape of transform's outputs, excl. the first batch dimension.
        ...
        if len(self.transforms) == self.num_transforms:
            raise RuntimeError("Adding more than {} transforms is not allowed.".format(self.num_transforms))
        ...
        if transform_output_shape[self.split_dim - 1] < 2:
            raise ValueError("Size of dimension {} must be at least 2.".format(self.split_dim))

        self.transforms.append(transform)

        if len(self.transforms) != self.num_transforms:  # Unless last transform.
            output_shape = list(transform_output_shape)
            output_shape[self.split_dim - 1] = (
                output_shape[self.split_dim - 1] + 1
            ) // 2
    ...
    def cascade(self, inputs, transforms, context):
        batch_size = inputs.shape[0]
        outputs = inputs
        total_logabsdet = inputs.new_zeros(batch_size)
    ...
    def inverse(self, inputs, context=None):
        ...
        rev_inv_transforms = [
            transform.inverse for transform in self.transforms[::-1]
        ]

        split_indices = np.cumsum(
            [np.prod(shape) for shape in self.output_shapes]
        )
        split_indices = np.insert(split_indices, 0, 0)

        split_inputs = [
            inputs[:, split_indices[i]: split_indices[i + 1]].view(
                -1, *self.output_shapes[i]
