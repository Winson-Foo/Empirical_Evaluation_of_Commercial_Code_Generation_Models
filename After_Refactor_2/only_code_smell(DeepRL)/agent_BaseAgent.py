...
from utils import get_logger, close_obj
...
class BaseAgent:
    ...
    def switch_task(self):
        ...
        if not tasks:
            return
        segs = np.linspace(0, self.config.max_steps, len(tasks) + 1)
        if self.total_steps > segs[self.task_ind + 1]:
            self.task_ind += 1
            self.task = tasks[self.task_ind]
