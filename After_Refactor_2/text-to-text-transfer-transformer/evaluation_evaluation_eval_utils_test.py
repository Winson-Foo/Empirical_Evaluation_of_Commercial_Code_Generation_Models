...
class EvalUtilsTest(absltest.TestCase):
    ...
    def test_parse_events_files(self):
        tensorboard_summary_dir = self.create_tempdir()
        with tf.Graph().as_default():
            summary_writer = tf.summary.FileWriter(tensorboard_summary_dir.full_path)
            tags = [
        ...
        events = eval_utils.parse_events_files(tensorboard_summary_dir.full_path)
    ...
    def test_parse_events_files_seqio(self):
        tensorboard_summary_dir = self.create_tempdir()
        metrics = [
            {"accuracy": seqio.metrics.Scalar(1.)},
            {"accuracy": seqio.metrics.Scalar(2.)}
        ]
        steps = [20, 30]

        logger = seqio.TensorBoardLoggerV1(tensorboard_summary_dir.full_path)
        ...
        events = eval_utils.parse_events_files(
            os.path.join(tensorboard_summary_dir.full_path, "foo_task"),
    ...
    def test_glue_average(self):
        metric_names = eval_utils.METRIC_NAMES.items()
        glue_metric_names = [
            v.name for k, v in metric_names if k.startswith("glue") and "average" not in k
        ]
        super_glue_metric_names = [
            v.name for k, v in metric_names if k.startswith("super") and "average" not in k
        ]
