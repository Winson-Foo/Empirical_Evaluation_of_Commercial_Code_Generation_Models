...
# Split the code into smaller functions or methods
def visualize_movie(movie):
    pl.ion()
    movie.play()

def select_roi(movie):
    print("Please draw a polygon delimiting the ROI on the image that will be displayed after the image; press enter when done")
    mask = np.array(behavior.select_roi(np.median(movie[::100], 0), 1)[0], np.float32)
    return mask

def extract_motor_components_OF(movie, n_components, mask, resize_fact, only_magnitude, method_factorization, max_iter_DL):
    spatial_filter_, time_trace_, of_or = cm.behavior.behavior.extract_motor_components_OF(movie, n_components, mask=mask,
                                                                                           resize_fact=resize_fact, only_magnitude=only_magnitude, verbose=True,
                                                                                           method_factorization=method_factorization, max_iter_DL=max_iter_DL)
    return spatial_filter_, time_trace_, of_or

def extract_magnitude_and_angle_from_OF(spatial_filter_, time_trace_, of_or, num_std_mag_for_angle, only_magnitude):
    ...
    return mags, dircts, dircts_thresh, spatial_masks_thrs
...
if __name__ == "__main__":
    ...
    spatial_filter_, time_trace_, of_or = extract_motor_components_OF(m, n_components, mask, resize_fact, only_magnitude, method_factorization, max_iter_DL)

    mags, dircts, dircts_thresh, spatial_masks_thrs = extract_magnitude_and_angle_from_OF(spatial_filter_, time_trace_, of_or, num_std_mag_for_angle, only_magnitude)
