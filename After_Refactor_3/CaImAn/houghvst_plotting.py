...
def plot_image(ax, img, vmin, vmax, cmap):
    ax.imshow(img, vmin=vmin, vmax=vmax, cmap=cmap)


def plot_cuts(ax, img, cuts):
    for c in cuts:
        if c.orientation == 'v':
...
def transversal_cuts(img, cuts, cmap='gray', normalize=False, axes=None):
    ...
    if axes is None:
        ...
        ax = axes[0]
    plot_image(ax, img, vmin, vmax, cmap)
    plot_cuts(ax, img, cuts)
...
def plot_patches_overlay(img, patches, selection=[], cmap='gray', normalize=False):
    ...
    plot_image(grid[0], img, vmin, vmax, cmap)
    grid[0].axis('off')
