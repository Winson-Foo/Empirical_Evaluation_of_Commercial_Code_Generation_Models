...
def patch_selection(img, n_patches, patch_size=32, overlapping=False):
    ...
    if not overlapping:
        patch_idx, patches, masks = select_nonoverlapping_patches(img, idx, shape, n_patches, patch_size)
    else:
        patch_idx, patches = select_overlapping_patches(img, idx, shape, n_patches, patch_size)

    return patch_idx, patches


def select_nonoverlapping_patches(img, idx, shape, n_patches, patch_size):
...
def select_overlapping_patches(img, idx, shape, n_patches, patch_size):
