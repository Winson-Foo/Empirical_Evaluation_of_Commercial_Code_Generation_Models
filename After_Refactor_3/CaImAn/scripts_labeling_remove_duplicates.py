...
def merge_regions(foldernames):
    for foldername in foldernames:
        current_folder = os.path.join('/mnt/ceph/neuro/labeling', foldername, 'regions')
        img_shape = cm.load(os.path.join('/mnt/ceph/neuro/labeling', foldername, 'projections/correlation_image.tif')).shape
        filenames = glob.glob(os.path.join(current_folder, '*active*regions.zip'))
        
        for filename in filenames:
            ind_duplicates, ind_keep = detect_duplicates(filename, 0.25, FOV=img_shape)
            ...
            if not ind_duplicates:
                ...
                nf_merge_roi_zip([filename], [ind_duplicates], filename[:-4] + '_copy')
