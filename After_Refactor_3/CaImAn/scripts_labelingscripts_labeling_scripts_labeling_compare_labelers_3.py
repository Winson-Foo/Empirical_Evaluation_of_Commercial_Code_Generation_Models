...
def set_opencv_num_threads():
    try:
        cv2.setNumThreads(1)
    except:
        print('OpenCV is naturally single threaded')

def load_ipython_autoreload_extension():
    try:
        if __IPYTHON__:
            print((1))
...
def compare_labelers_consensus(folders_out):
    ...
    for folder_out in folders_out:
        ...
        for fl2 in fls:
            ...
            lab1, lab2 = os.path.split(fl1)[-1][:-4], os.path.split(fl2)[-1][:-4]
...
def extract_f1_scores(folders_out):
    f1s = []
    names = []
...
def plot_labeler_matches(folders_out):
    counter = 1
    ...
    for folder_out in folders_out:
        ...
        with np.load(os.path.join(folder_in, 'comparison_labelers.npz')) as ld:
            ...
            for fl_m in [os.path.join(folder_in, '*_matches.zip'), os.path.join(folder_in, '*1_mismatches.zip'), os.path.join(folder_in, '*0_mismatches.zip')]:
                ...
                pl.axis('off')
                pl.xlabel(os.path.split(fl_m)[-1])

            pl.title(os.path.split(folder_out)[-1])
            pl.pause(1)

def refactor_codebase():
    set_opencv_num_threads()
    load_ipython_autoreload_extension()
    ...
    g = extract_f1_scores(folders_out)
    plot_labeler_matches(folders_out)
