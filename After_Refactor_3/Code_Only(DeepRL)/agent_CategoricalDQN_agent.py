...
class CategoricalDQNAgent(DQNAgent):
    ...
    def init_config_values(self):
        ...
        self.delta_atom = (self.config.categorical_v_max - self.config.categorical_v_min) / float(
            self.config.categorical_n_atoms - 1)
    ...
    def eval_step(self, state):
        ...
        with self.config.state_normalizer.unset_read_only():
            return action
