...
class TD3Agent(BaseAgent):
    ...
    def warmup(self):
        if self.total_steps < self.config.warm_up:
            return [self.task.action_space.sample()]
        else:
            state, action, _, _, _= self.replay.sample()
            action = self.target_network(state)
            action += self.random_process.sample()
            action = np.clip(action, self.task.action_space.low, self.task.action_space.high)
            return action

    def feed_and_step(self, next_state, reward, done, info):
    ...
    def step(self):
        ...
        if self.state is None:
            ...
            self.action = self.warmup()

        next_state, reward, done, info = self.task.step(self.action)
        next_state = self.config.state_normalizer(next_state)
        self.feed_and_step(next_state, reward, done, info)
        self.action = self.network(self.state)
        self.action = to_np(self.action)
        self.action += self.random_process.sample()
