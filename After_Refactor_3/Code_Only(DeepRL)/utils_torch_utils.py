...
class Grad:
    ...
    def add(self, op):
        assert isinstance(op, Grad) or isinstance(op, torch.nn.Module)
    ...
    def clone(self):
        grads_copy = [grad.clone() for grad in self.grads]
        return Grad(grads=grads_copy)
...
class Grads:
    ...
    def add(self, op):
        assert isinstance(op, Grads) or np.isscalar(op) or isinstance(op, torch.Tensor)
