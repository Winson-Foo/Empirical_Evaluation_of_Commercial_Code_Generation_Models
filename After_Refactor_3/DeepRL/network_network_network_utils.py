...
def layer_init(layer: nn.Module, w_scale: float = 1.0) -> nn.Module:
    """Initialize a neural network layer with orthogonal weights and zero biases."""
...
class NoisyLinear(nn.Module):
    """
    A linear layer with added noise to improve exploration
    during training. Based on the implementation in the
    RL Adventures tutorial on Noisy DQN.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Feed forward through the noisy linear layer."""
        if self.training:
    ...
    def reset_noise(self) -> None:
        """Reset the noise added to the weights and biases."""
        self.noise_in.normal_(std=noisy_layer_standard_deviation)
        self.noise_out_weight.normal_(std=noisy_layer_standard_deviation)
        self.noise_out_bias.normal_(std=noisy_layer_standard_deviation)
    ...
    def transform_noise(self, x: torch.Tensor) -> torch.Tensor:
        """
        Transform Gaussian noise into signed, square-root-transformed noise
        for added exploration during training.
        """
