    ...
    subprocess.call(
        [
            "sox {} -r {} -b 16 -c 1 {} trim {} ={}".format(
                src_sph_file, str(sample_rate), target_wav_file, start_time, end_time
            )
        ],
        shell=True,
    )
...
class DataProcessor:
        ...
        for sph_file in tqdm(entries, total=len(entries)):
            ...
            for utterance_id, utterance in enumerate(all_utterances):
                target_wav_file = os.path.join(
                    wav_dir, "{}_{}.wav".format(utterance["filename"], str(utterance_id))
                )
                target_txt_file = os.path.join(
                    txt_dir, "{}_{}.txt".format(utterance["filename"], str(utterance_id))
                )
                cut_utterance(
                    sph_file_full,
                    target_wav_file,
    ...
    def process(self):
        target_dl_dir = self.args.target_dir
        ...
        if self.args.tar_path and os.path.exists(self.args.tar_path):
            target_file = self.args.tar_path
        else:
        ...
        self.prepare_dirs(train_ted_dir)
        self.prepare_dirs(val_ted_dir)
        self.prepare_dirs(test_ted_dir)
        print("Creating manifests...")

        create_manifest(
            data_path=train_ted_dir,
            output_name="ted_train_manifest.json",
            manifest_path=self.args.manifest_dir,
...
def main():
    ...
    parser.add_argument(
        "--target-dir", default="TEDLIUM_dataset/", type=str, help="Directory to store the dataset."
    )
