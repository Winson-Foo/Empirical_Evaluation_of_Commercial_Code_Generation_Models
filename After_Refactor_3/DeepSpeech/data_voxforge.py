...
VOXFORGE_BASE_URL = 'http://www.repository.voxforge1.org/downloads/SpeechCorpus/Trunk/Audio/Main'
VOXFORGE_URL_16kHz = f"{VOXFORGE_BASE_URL}/16kHz_16bit/"
...
def download_and_extract_sample(recording_name, url, target_folder):
    """
...
def download_voxforge_dataset():
    ...
    for f in tqdm(all_files, total=len(all_files)):
        ...
        url = f"{VOXFORGE_URL_16kHz}{f}"
        download_and_extract_sample(recording_name, url, target_dir)

def create_voxforge_manifest():
    print('Creating manifests...')
    create_manifest(
        data_path=args.target_dir,
...
if __name__ == '__main__':
    args = get_arguments()
    download_voxforge_dataset()
    create_voxforge_manifest()