# TD3 Agent for Reinforcement Learning Tasks
# Author: Shangtong Zhang (zhangshangtong.cpp@gmail.com)
...
class TD3Agent(BaseAgent):
    """
    Twin Delayed DDPG (TD3) agent for reinforcement learning tasks.
    """
    def __init__(self, config):
        """
        Initialize the TD3 agent with the given configuration.
        """
    ...
    def soft_update(self, target, src):
        """
        Update the target network using a soft update rule.

        Args:
            target: The target network to be updated.
            src: The source network to copy the weights from.
        """
    ...
    def eval_step(self, state):
        """
        Evaluate a given state using the current policy network.

        Args:
            state: The state to be evaluated.

        Returns:
            The action to be taken according to the current policy network.
        """
    ...
    def step(self):
        """
        Perform a single step in the environment using the TD3 agent.
        """
        config = self.config
