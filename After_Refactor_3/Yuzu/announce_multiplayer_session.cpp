...
AnnounceMultiplayerSession::AnnounceMultiplayerSession(Network::RoomNetwork& room_network_)
    : room_network(room_network_), backend(nullptr), registered(false), announce_multiplayer_thread(nullptr) {
#ifdef ENABLE_WEB_SERVICE
...
void AnnounceMultiplayerSession::UpdateBackendData(const std::shared_ptr<Network::Room>& room) {
    const auto& memberlist = room->GetRoomMemberList();
    backend->SetRoomInformation(room->GetRoomInformation().name, room->GetRoomInformation().description,
                                room->GetRoomInformation().port, room->GetRoomInformation().member_slots,
                                Network::network_version, room->HasPassword(),
                                room->GetRoomInformation().preferred_game);
