...
static constexpr u32 kErrorCategoryOffset = 2000;
...
struct ErrorCode {
    ...
    static ErrorCode FromResult(Result result) {
        return {
            .error_category{kErrorCategoryOffset + static_cast<u32>(result.module.Value())},
            .error_number{result.description.Value()},
        };
    }

    constexpr Result ToResult() const {
        return Result{static_cast<ErrorModule>(error_category - kErrorCategoryOffset), error_number};
    }
};
...
constexpr auto kShowErrorSize = sizeof(ShowError);
constexpr auto kShowErrorRecordSize = sizeof(ShowErrorRecord);
constexpr auto kSystemErrorArgSize = sizeof(SystemErrorArg);
constexpr auto kApplicationErrorArgSize = sizeof(ApplicationErrorArg);
} // Anonymous namespace
...
void Error::Execute() {
    ...
    case ErrorAppletMode::ShowErrorRecord:
        reporter.SaveErrorReport(title_id, error_code, fmt::format("{:016X}", args->error_record.posix_time));
