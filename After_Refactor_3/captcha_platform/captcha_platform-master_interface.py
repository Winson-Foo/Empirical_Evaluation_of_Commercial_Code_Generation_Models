...
class Interface:
    def __init__(self, graph_session: GraphSession):
        ...
        if self.graph_sess.loaded:
            self.session = self.graph_sess.session
            self.dense_decoded = self.session.graph.get_tensor_by_name("dense_decoded:0")
            self.input_tensor = self.session.graph.get_tensor_by_name('input:0')
