...
from middleware import color_extract
from middleware import ImageUtils
...
class Predict(grpc_pb2_grpc.PredictServicer):
    ...
    def __init__(self, system_config, interface_manager):
        super().__init__()
    ...
    def predict(self, request, context):
        ...
        image_size = self.image_utils.size_of_image(image_sample)
        ...
        image_batch, status = self.image_utils.get_image_batch(interface.model_conf, bytes_batch)
...
def serve():
    ...
    grpc_pb2_grpc.add_PredictServicer_to_server(Predict(system_config, interface_manager), server)
