...
def calculate_coordinate_ranges(param, is_range=True, is_integer=True):
...
def parse_multi_image(image_bytes, param_group):
    ...
    for param in param_group:
        pos_ranges = calculate_coordinate_ranges(param, True, True)
...
def get_coordinate_by_label(label: str, param_group, title_index=None):
    if title_index is None:
        title_index = [0]
    param = param_group[-1]
    coord_map = calculate_coordinate_ranges(param, is_range=False, is_integer=True)
    index_group = get_pair_index_by_label(label=label, title_index=title_index)
    return [coord_map[i] for i in index_group]

def get_pair_index_by_label(label: str, title_index=None):
    if title_index is None:
        title_index = [0]
...
if __name__ == '__main__':
    ...
    for name in os.listdir(root_dir):
        ...
        with open(path, "rb") as f:
            file_bytes = [f.read()]
        group = parse_multi_image(file_bytes, _param_group)
        for b in group:
