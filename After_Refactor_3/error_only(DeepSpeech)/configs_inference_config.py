...
class LMConfig:
    ...
    lm_path: Optional[str] = None  # Path to an (optional) kenlm language model for use with beam search (req\'d with trie)
...
class TranscribeConfig(InferenceConfig):
    audio_path: Optional[str] = None  # Audio file to predict on
    offsets: bool = False  # Returns time offset information
    chunk_size_seconds: Optional[float] = None # default value to transcribe the whole audio at once


@dataclass
class EvalConfig(InferenceConfig):
    test_path: Optional[str] = None  # Path to validation manifest csv or folder
    verbose: bool = True  # Print out decoded output and error of each sample
    save_output: Optional[str] = None  # Saves output of model from test to this file_path
