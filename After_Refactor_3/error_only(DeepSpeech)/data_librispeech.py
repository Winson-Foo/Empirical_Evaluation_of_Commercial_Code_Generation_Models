...
def _process_file(wav_dir, txt_dir, base_filename, root_dir):
    ...
    if not os.path.exists(full_recording_path):
        print(f"{full_recording_path} does not exist.")
        return
    ...
    try:
        ...
        print(f"Error processing file {full_recording_path}: {e}")
        return
    # process transcript
    ...
    if not os.path.exists(transcript_file):
        print(f"Transcript file {transcript_file} does not exist.")
        return
    ...
    for split_type, lst_libri_urls in LIBRI_SPEECH_URLS.items():
        ...
        if split_type == 'train':  
            ...
            try:
                ...
                print(f"Error creating manifest for {split_type}: {e}")
                return
        else:
            try:
                create_manifest(
