...
parser.add_argument("--tar-path", default=None, type=str, help="Path to the TEDLIUM_release tar if downloaded (Optional).")
    ...
    for sph_file in tqdm(entries, total=len(entries)):
        ...
        for utterance_id, utterance in enumerate(all_utterances):
            ...
            try:
                ...
                print("Error while processing file: %s" % sph_file_full, sys.exc_info()[0])
                continue
...
def main():
    ...
    if args.manifest_dir is None:
        raise ValueError("Argument --manifest-dir is required")
    if not os.path.exists(args.manifest_dir):
        os.makedirs(args.manifest_dir)
