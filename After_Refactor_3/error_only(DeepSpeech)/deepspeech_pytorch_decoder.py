...
class BeamCTCDecoder(Decoder):
    ...
    def decode(self, probs, sizes=None):
        ...
        if not isinstance(probs, torch.Tensor): raise TypeError("Probs should be a tensor.")
        
        probs = probs.cpu()
        try:
            out, scores, offsets, seq_lens = self._decoder.decode(probs, sizes, return_offsets=True)
        except TypeError:
...
class GreedyDecoder(Decoder):
    ...
    def decode(self, probs, sizes=None):
        ...
        try:
            _, max_probs = torch.max(probs, 2, keepdim=True)
        except TypeError:
