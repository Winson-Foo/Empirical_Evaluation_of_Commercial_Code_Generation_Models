...
class PretrainedSmokeTest(DeepSpeechSmokeTest):
    ...
    def test_pretrained_eval_inference(self):
        ...
        try:
            ...
            print("Error while downloading LM:", e)
            return
        
        try:
            with gzip.open(os.path.basename(lm_path), 'rt', encoding='utf8') as f:
                lm_path_str = os.path.splitext(lm_path)[0]
                with open(lm_path_str, 'w') as fout:
                    fout.write(f.read())
        except Exception as e:
            print("Error while unzipping LM:", e)
            return
        ...
        for pretrained_url in pretrained_urls:
            ...
            try:
                ...
                print("Error while downloading pretrained model:", e)
                continue
            ...
            lm_configs = [
                ...
                LMConfig(
                    decoder_type=DecoderType.beam,
                    lm_path=os.path.basename(lm_path_str),
                    alpha=1,
                    beta=1
            ...
            for lm_config in lm_configs:
                try:
                    ...
                    print("Error while evaluating model:", e)
                    continue
                try:
                    self.inference(
                        test_path=test_manifest,
