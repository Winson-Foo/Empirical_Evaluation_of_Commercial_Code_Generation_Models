...
from .base import Model, T, Attack, get_is_adversarial, get_criterion, raise_if_kwargs, verify_input_bounds
from .spatial_attack_transformations import rotate_and_shift

class SpatialAttack(Attack):
    def __init__(self, max_translation: float = 3, max_rotation: float = 30,
                 num_translations: int = 5, num_rotations: int = 5, grid_search: bool = True,
                 random_steps: int = 100):
        self.max_trans = max_translation
        self.max_rot = max_rotation
        self.grid_search = grid_search
        self.num_trans = num_translations if grid_search else None
        self.num_rots = num_rotations if grid_search else None
        self.random_steps = random_steps if not grid_search else None

    def __call__(self, model: Model, inputs: T, criterion: Any, **kwargs: Any) -> Tuple[T, T, T]:
    ...
    def run(self, model: Model, inputs: T, criterion: Union[Criterion, T], **kwargs: Any) -> T:
        ...
        def grid_search_generator() -> Generator:
        ...
        def random_search_generator() -> Generator:
