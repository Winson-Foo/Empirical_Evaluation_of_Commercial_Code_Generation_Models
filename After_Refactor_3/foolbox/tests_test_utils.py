...
def test_samples(fmodel_and_data: ModelAndData, batchsize: int, dataset: str) -> None:
    ...
    data_format = fmodel.data_format if hasattr(fmodel, "data_format") else "channels_first"
    
    x, y = fbn.samples(fmodel, dataset=dataset, batchsize=batchsize, data_format=data_format)
