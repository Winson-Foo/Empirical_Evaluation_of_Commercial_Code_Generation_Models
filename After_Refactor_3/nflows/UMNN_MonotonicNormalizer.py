...
class MonotonicNormalizer(nn.Module):
    def __init__(self, integrand_net, cond_size: int, nb_steps: int = 20, solver: str = "CC"):
        super().__init__()
    ...
    def forward(self, x, h, context=None):
        ...
        if self.solver == "CC":
            z = NeuralIntegral.apply(x0, xT, self.integrand_net, flatten_sequence(self.integrand_net.parameters()),
                                     h, self.nb_steps) + z0
        elif self.solver == "CCParallel":
            z = ParallelNeuralIntegral.apply(x0, xT, self.integrand_net,
                                             flatten_sequence(self.integrand_net.parameters()),
                                             h, self.nb_steps) + z0
        else:
            return None
