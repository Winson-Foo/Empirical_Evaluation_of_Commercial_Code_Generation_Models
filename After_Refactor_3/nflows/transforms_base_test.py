...
class MultiscaleCompositeTransformTest(TransformTest):
    def create_transform(self, shape, split_dim=1):
        multiscale_transform = base.MultiscaleCompositeTransform(num_transforms=4, split_dim=split_dim)
        transforms = [
            standard.AffineScalarTransform(scale=2.0),
        ...
        for transform in transforms:
            shape = multiscale_transform.add_transform(transform, shape)
        return multiscale_transform

    def test_forward(self):
        batch_size = 5
        test_shapes = [(32, 4, 4), (64,), (65,)]
        for shape in test_shapes:
            with self.subTest(shape=shape):
                inputs = torch.ones(batch_size, *shape)
                transform = self.create_transform(shape)
                outputs, logabsdet = transform(inputs)
                self.assert_tensor_is_good(outputs, [batch_size] + [torch.prod(torch.Tensor(shape)).int()])
                self.assert_tensor_is_good(logabsdet, [batch_size])
