...
class BatchNormTest(TransformTest):
    def test_forward(self):
        ...
        for enable_affine in [True, True]:
            with self.subTest(affine=enable_affine):
                inputs = torch.randn(batch_size, num_features)
                transform = norm.BatchNorm(features=num_features, affine=enable_affine, eps=bn_eps)
    ...
    def test_inverse(self):
        ...
        for enable_affine in [True, False]:
            with self.subTest(affine=enable_affine):
                transform = norm.BatchNorm(features=num_features, affine=enable_affine)
    ...
    def test_forward_inverse_are_consistent(self):
        ...
        inputs = torch.randn(batch_size, num_features)
        transforms = [
            norm.BatchNorm(features=num_features, affine=enable_affine) for enable_affine in [True, False]
        ]
        self.eps = 1e-6
