    ...
    def test_matrix(self):
        for num_transforms in self.num_transforms:
            with self.subTest(num_transforms=num_transforms):
                ...
                self.eps = 1e-5
                self.assertTrue(torch.allclose(matrix @ matrix.t(), torch.eye(self.features)))
                self.assertTrue(torch.allclose(matrix.t() @ matrix, torch.eye(self.features)))
                self.assertTrue(torch.allclose(matrix.t(), torch.inverse(matrix)))
