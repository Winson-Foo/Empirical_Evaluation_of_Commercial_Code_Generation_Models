...
class QRLinearTest(TransformTest):
    ...
    def test_forward_no_cache(self):
        ...
        assert torch.all(outputs.eq(outputs_ref))
        assert torch.all(logabsdet.eq(logabsdet_ref))
    ...
    def test_weight(self):
        ...
        assert torch.all(weight.eq(self.weight))
    ...
    def test_weight_inverse(self):
        ...
        assert torch.all(weight_inverse.eq(self.weight_inverse))

    def test_logabsdet(self):
        logabsdet = self.transform.logabsdet()
        self.assert_tensor_is_good(logabsdet, [])
        assert torch.all(logabsdet.eq(self.logabsdet))
