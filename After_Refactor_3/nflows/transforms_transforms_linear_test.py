...
class LinearTest(TransformTest):
    def setUp(self):
        ...
        self.weight = torch.randn(self.features, self.features)
        self.inverse = torch.randn(self.features, self.features)
        self.logabsdet = torch.randn(1)
        self.bias = torch.randn(self.features)  # Just so bias isn't zero.

        self.transform = Linear(self.features)
        self.transform.bias.data = self.bias

        self.inputs = torch.randn(self.batch_size, self.features)
        self.outputs_fwd = self.inputs @ self.weight.t() + self.bias
        self.outputs_inv = (self.inputs - self.bias) @ self.inverse.t()
        self.logabsdet_fwd = self.logabsdet * torch.ones(self.batch_size)
        self.logabsdet_inv = (-self.logabsdet) * torch.ones(self.batch_size)
        ...
        self.transform.weight = MagicMock(return_value=self.weight)
        self.transform.weight_inverse = MagicMock(return_value=self.inverse)
        self.transform.logabsdet = MagicMock(return_value=self.logabsdet)
