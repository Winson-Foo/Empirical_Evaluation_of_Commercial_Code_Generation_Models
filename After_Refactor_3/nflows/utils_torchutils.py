...
from nflows.utils.typechecks import is_positive_int, is_nonnegative_int


def tile(x: torch.Tensor, n: int) -> torch.Tensor:
    """Tiles the tensor `x` `n` times along the leading dimension."""
...
def split_leading_dim(x: torch.Tensor, shape: tuple) -> torch.Tensor:
...
def repeat_rows(x: torch.Tensor, num_reps: int) -> torch.Tensor:
    """Each row of tensor `x` is repeated `num_reps` times along the leading dimension."""
...
def get_num_parameters(model: torch.nn.Module) -> int:
    """
    Returns the number of trainable parameters in a model of type torch.nn.Module.

    :param model: torch.nn.Module containing trainable parameters
...
def get_temperature(max_value: float, bound: float = 1 - 1e-3) -> float:
    """
