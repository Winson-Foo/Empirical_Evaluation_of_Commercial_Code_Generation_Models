...
class OptionCriticAgent(BaseAgent):
    def __init__(self, config):
        ...
        self.is_initial_states = torch.ones((config.num_workers)).byte()
    ...
    def sample_option(self, prediction, epsilon, prev_option, is_initial_states):
        ...
        num_actions = q_option.size(1)
        # Calculate greedy actions probabilities
        pi_option = torch.zeros_like(q_option).add(epsilon / num_actions)
    ...
    def collect_rollout(self, storage):
        config = self.config
