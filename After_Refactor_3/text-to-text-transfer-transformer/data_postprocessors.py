...
def string_to_float(string, default=-1., **unused_kwargs):
  """Converts string to float, using the default value when conversion is not possible."""
  try:
    return float(string)
  except ValueError:
    return default
...
def string_label_to_class_id(string_label, label_classes, default=-1, **unused_kwargs):
  """Returns the index of `string_label` in `label_classes` or `default` if not found."""
...
def multirc(string_label, example=None, is_target=False):
  """Returns a dictionary containing the class with the question index for grouping."""
  res = {
      "value": string_label_to_class_id(string_label, example=example, label_classes=("False", "True"))
  }
...
def record(answer, example=None, is_target=False):
  """Returns a dictionary with the answer or all answers + grouping key for a target."""
  if is_target:
    return {
...
def qa(answer, example=None, is_target=False):
  """Returns the answer or all answers if the full example is provided."""
  if is_target:
...
def span_qa(answer, example=None, is_target=False):
  """Returns the answer or a dictionary with answers and context if the example is provided."""
  if is_target:
    return {
...
def wsc_simple(prediction, example=None, is_target=False):
  ...
  # Initialize a set of determiners.
  determiners = {
  ...
  if not prediction:
    # We don't want an empty prediction to accidentally return 0 and spuriously match the label.
    return -1

  # We aren't using the label but rather using the extracted referent so that we can see if the prediction is equivalent to the referent.
  referent = clean(example["targets_pretokenized"])

  if ("'" in prediction) != ("'" in referent):
    # Make sure we don't mark cases where the prediction is "Bob" and the referent is "Bob's hat" as predicting the referent.
    predicted_referent = False
  else:
    prediction_words = set(prediction.split(" "))
    referent_words = set(referent.split(" "))

    # Handle cases where the prediction is "fuzzy bunny" and the referent is "bunny".
