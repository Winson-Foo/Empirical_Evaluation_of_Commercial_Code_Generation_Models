...
flags.DEFINE_string("task", None, "A registered Task.", required=True)
...
flags.DEFINE_multi_string("module_import", _DEFAULT_MODULE_IMPORTS, "Modules to import. Use this when your Task or is defined outside of the T5 codebase so that it is registered.")
...
flags.DEFINE_multi_string("delimiters", _DEFAULT_DELIMITERS, "Optional. Delimiters to highlight in terminal output when pretty is enabled.")
  ...
  def _example_to_string(ex):
    ...
    for k in ex.keys():
      ...
      if FLAGS.detokenize:
        ...
        if FLAGS.apply_postprocess_fn and k == "targets" and hasattr(task_or_mixture, "postprocess_fn"):
